...
if (typeof GasLog === 'undefined') {
  // GasL Initialization. (only if not initialized yet.)
  eval(
    UrlFetchApp.fetch(
      'https://raw.githubusercontent.com/zixia/gasl/master/src/gas-log-lib.js'
    ).getContentText()
  );
...
if (typeof GasTap === 'undefined') {
  eval(
    UrlFetchApp.fetch(
      'https://raw.githubusercontent.com/zixia/gast/v0.2.0/src/gas-tap-lib.js'
    ).getContentText()
  );
} // Class GasTap is ready for use now!
```
